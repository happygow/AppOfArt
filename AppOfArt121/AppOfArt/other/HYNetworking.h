...
// 项目打包上线都不会打印日志，因此可放心。
#ifdef DEBUG
#define HYLog(s, ... ) NSLog( @"[%@ in line %d] ===============>%@", [[NSString stringWithUTF8String:__FILE__] lastPathComponent], __LINE__, [NSString stringWithFormat:(s), ##__VA_ARGS__] )
#else
#define HYLog(s, ... )
#endif
...
typedef HYDownloadProgress HYGetProgress;
typedef HYDownloadProgress HYPostProgress;
 ...
 */
typedef void (^HYUploadProgress)(int64_t bytesWritten,
                                  int64_t totalBytesWritten);

typedef NS_ENUM(NSUInteger, HYResponseType) {
  kHYResponseTypeJSON = 1, // 默认
  kHYResponseTypeXML  = 2, // XML
  // 特殊情况下，一转换服务器就无法识别的，默认会尝试转换成JSON，若失败则需要自己去转换
  kHYResponseTypeData = 3
};

typedef NS_ENUM(NSUInteger, HYRequestType) {
  kHYRequestTypeJSON = 1, // 默认
  kHYRequestTypePlainText  = 2 // 普通text/html
};

typedef NS_ENUM(NSInteger, HYNetworkStatus) {
    kHYNetworkStatusUnknown          = -1,//未知网络
    kHYNetworkStatusNotReachable     = 0,//网络无连接
    kHYNetworkStatusReachableViaWWAN = 1,//2，3，4G网络
    kHYNetworkStatusReachableViaWiFi = 2,//WIFI网络
};
 ...
 */
+ (void)configRequestType:(HYRequestType)requestType
             responseType:(HYResponseType)responseType
 ...
 *
 *	取消某个请求。如果是要取消某个请求，最好是引用接口所返回来的HYURLSessionTask对象，
 ...
 */
+ (HYURLSessionTask *)getWithUrl:(NSString *)url
...
+ (HYURLSessionTask *)getWithUrl:(NSString *)url
                     refreshCache:(BOOL)refreshCache
                           params:(NSDictionary *)params
                         progress:(HYGetProgress)progress
                          success:(HYResponseSuccess)success
                             fail:(HYResponseFail)fail;
 ...
 *  @return 返回的对象中有可取消请求的API
 */
+ (HYURLSessionTask *)postWithUrl:(NSString *)url
                      refreshCache:(BOOL)refreshCache
                            params:(NSDictionary *)params
                           success:(HYResponseSuccess)success
                              fail:(HYResponseFail)fail;
+ (HYURLSessionTask *)postWithUrl:(NSString *)url
                      refreshCache:(BOOL)refreshCache
                            params:(NSDictionary *)params
                          progress:(HYPostProgress)progress
                           success:(HYResponseSuccess)success
                              fail:(HYResponseFail)fail;
/**
...
+ (HYURLSessionTask *)uploadWithImage:(UIImage *)image
                            ...
                            parameters:(NSDictionary *)parameters
                              progress:(HYUploadProgress)progress
                               success:(HYResponseSuccess)success
                                  fail:(HYResponseFail)fail;
